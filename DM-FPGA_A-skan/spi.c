...
//------------------------------------------------------------------------------
//
//  Function:  SPI_CSx_ManualMode
//
//  This function configures the chip select to be in manual mode, rather than the default automatic mode.
//
BOOL SPI_CSx_ManualMode(DWORD context, DWORD enable)
{
    BOOL rc = FALSE;
    SPI_INSTANCE *pInstance = (SPI_INSTANCE*)context;
    SPI_DEVICE *pDevice;
	  DWORD regData=0;
    DEBUGMSG(ZONE_FUNCTION, (L"+SPI_CSx_ManualMode enable = 0x%x\r\n", enable));

    // Check if we get correct context
    if (pInstance == NULL || pInstance->cookie != SPI_INSTANCE_COOKIE) {
        DEBUGMSG (ZONE_ERROR, (L"ERROR: SPI_CSx_ManualMode: "
            L"Incorrect context paramer\r\n"
        ));
        goto cleanUp;
    }
...
#if 0
	SPILockController((HANDLE)context, 500);

    // Check to see if we have exclusive access
    if (!pInstance->exclusiveAccess)
    {
        DEBUGMSG (ZONE_ERROR, (L"ERROR: SPI_CSx_ManualMode: Not in single access mode!!\r\n"));
        goto cleanUp;
    }
#endif
    ...
    // Set this driver to Active Mode
    SetSPIPower(pDevice, D0);
	regData = INREG32(&pDevice->pSPIRegs->MCSPI_MODULCTRL);
    // Configure the channel
 	if (enable)
	{
		//enable manual cs assertion
		SETREG32(&pDevice->pSPIRegs->MCSPI_MODULCTRL, MCSPI_SINGLE_BIT);	
		CLRREG32(&pDevice->pSPIRegs->MCSPI_MODULCTRL, MCSPI_MS_BIT);
	}
	else	
	{
		// disable manual cs, automatic
		CLRREG32(&pDevice->pSPIRegs->MCSPI_MODULCTRL, MCSPI_MS_BIT);	
		SETREG32(&pDevice->pSPIRegs->MCSPI_MODULCTRL, MCSPI_SINGLE_BIT);	
	}
...
#if 0
	SPIUnlockController((HANDLE)context);
#endif
...
//------------------------------------------------------------------------------
//
//  Function:  SPI_CSx_Force
//
//  This function the drives the chip select line, csx to the level passed in.
//
BOOL SPI_CSx_Force(DWORD context, DWORD address, DWORD csxLevel)
{
    BOOL rc = FALSE;
    SPI_INSTANCE *pInstance = (SPI_INSTANCE*)context;
    SPI_DEVICE *pDevice;
	  DWORD regData=0, ePol=0;

    DEBUGMSG(ZONE_FUNCTION, (L"+SPI_CSx_Force csxLevel = 0x%x\r\n", csxLevel));

    // Check if we get correct context
    if (pInstance == NULL || pInstance->cookie != SPI_INSTANCE_COOKIE) {
        DEBUGMSG (ZONE_ERROR, (L"ERROR: SPI_CSx_Force: "
            L"Incorrect context paramer\r\n"
        ));
        goto cleanUp;
    }
    ...
    if (!pInstance->exclusiveAccess)
    {
        DEBUGMSG (ZONE_ERROR, (L"ERROR: SPI_CSx_Force: Not in single access mode!!\r\n"));
        goto cleanUp;
    }
#endif

//RETAILMSG(1, (L" %%%%%%%%%%%%% SPI_CSx_Force Step 1 \r\n"));
    ...
    if ((regData & MCSPI_SINGLE_BIT) == 0){
        DEBUGMSG (ZONE_ERROR, (L"ERROR: SPI_CSx_Force: "
            L"SINGLE bit in MODULCTRL must be set for CSx\r\n"
        ));
        goto cleanUp;
    }
    ...
    switch( address )
    {
		...
		ePol = INREG32(&pInstance->pSPIChannelRegs->MCSPI_CHCONF) & MCSPI_CSPOLARITY_ACTIVELOW ;

		if (csxLevel == TRUE)
		{
			if (ePol)	
				SETREG32(&pInstance->pSPIChannelRegs->MCSPI_CHCONF, MCSPI_CHCONF_FORCE);	// force cs high
			else	
				CLRREG32(&pInstance->pSPIChannelRegs->MCSPI_CHCONF, MCSPI_CHCONF_FORCE);	// force cs low		  	
		}		
		else
		 {
			if (ePol)	
				CLRREG32(&pInstance->pSPIChannelRegs->MCSPI_CHCONF, MCSPI_CHCONF_FORCE);	// force cs high
			else	
				SETREG32(&pInstance->pSPIChannelRegs->MCSPI_CHCONF, MCSPI_CHCONF_FORCE);	// force cs low		  	
		}		
								
		regData = INREG32(&pInstance->pSPIChannelRegs->MCSPI_CHCONF);
      break;
    ...
    )
{
    ...
    // Check if we get correct context
    if (pInstance == NULL || pInstance->cookie != SPI_INSTANCE_COOKIE) {
        DEBUGMSG (ZONE_ERROR, (L"ERROR: SPI_WriteRead: "
            L"Incorrect context paramer\r\n"
        ));
        return 0;
    }
...
//------------------------------------------------------------------------------
//
//  Function:  SPI_BT_WriteRead
//
//  This function writes and reads data to/from the device.
//  Both buffers must be the same length.
//  Note that this is only allowed for MCSPI_CHCONF_TRM_TXRX channels
//
//  LOGICPD:  This is a modified version of SPI_WriteRead to accomodate
//  timing requirements for Bluetooth over SPI.
//
DWORD
SPI_BT_WriteRead(
    DWORD context,
    ...
    )
{
    ...
    DWORD dwWait;
    DWORD regData=0;    
    UCHAR forceCSx = FALSE;

    DEBUGMSG(ZONE_FUNCTION, (L"+SPI_WriteRead(0x%08x, 0x%08x)\r\n", context, size));

    // Check if we get correct context
    if (pInstance == NULL || pInstance->cookie != SPI_INSTANCE_COOKIE) {
        DEBUGMSG (ZONE_ERROR, (L"ERROR: SPI_BT_WriteRead: "
            L"Incorrect context paramer\r\n"
        ));
        return 0;
    }
    ...
    if (!pInstance->exclusiveAccess)
    {
        DEBUGMSG (ZONE_ERROR, (L"ERROR: SPI_BT_WriteRead: Not in single access mode!!\r\n"));
        goto cleanUp;
    }
#endif
    ...
    if (regData & MCSPI_SINGLE_BIT)
    	forceCSx = TRUE;    
    
	if (forceCSx)
	{		
   		SPI_CSx_Force(context, pInstance->address, ASSERT_CSx);

		// according to the BT_SPI spec, on the first command only,
		// must wait a minimum 50 microseconds after the assertion if CS and 
		// before sending the data.  Sleep of 1 will provide 1 millisecond.
		if ( pOutData[6]==0x41 && pOutData[7]==0xFD )
			Sleep(1);
	}
    // Write out the data
    for( dwCount = 0; dwCount < size; )
    {
		...
		// according to the BT_SPI spec after fourth byte of the SPI_CFG command,
		// must wait a minimum 50 microseconds before sending the remaining packet.
		// Sleep of 1 will provide 1 millisecond.
		if ( pOutData[6]==0x41 && pOutData[7]==0xFD && dwCount == 3)
			Sleep(1);
        ...
        //only the SPI header has been read thus far.  The remaining number of bytes are determined by the
        //contents of the event message being returned.  Change the value of size once the information has been
        //validated.
        if (pOutData[0] == 0x03 &&		// this is a SPI read command
        	dwCount == 5)				// have at this point received the length info from BRF6300
        {
        	// determine HCI length information   
        	size = ((pInData[3] << 8 ) & 0xFF00) | (pInData[4] & 0x00FF) + dwCount;
    	}

    }

	if (forceCSx)
 		SPI_CSx_Force(context, pInstance->address, DEASSERT_CSx);
...
BOOL SPI_IOControl(
    ...
    case IOCTL_SPI_MANUAL_CS_CTRL:
        ...
        bRetVal = SPI_CSx_ManualMode(context,*pInBuffer);     

        break;   

    case IOCTL_SPI_FORCE_CSX:
       if (pOutSize != NULL) *pOutSize = sizeof(DWORD);
       if (pInBuffer == NULL || inSize < sizeof(IOCTL_SPI_CONFIGURE_IN)) {
            SetLastError(ERROR_INVALID_PARAMETER);
            break;
        ...
        bRetVal = SPI_CSx_Force(context,pConfig->address, pConfig->config);        
        break;        
    ...
    case IOCTL_SPI_BT_FULL_DUPLEX:
			if (pOutBuffer == NULL || outSize == 0){		// transmit data
	            SetLastError(ERROR_INVALID_PARAMETER);
				bRetVal = 0;
	            break;
			}

		   	// pOutBuffer - contains transmit data
		   	// pInBuffer - will contain receive data after SPI_WriteRead operation
        	bRetVal = SPI_BT_WriteRead(context, outSize, pOutBuffer, pInBuffer, inSize);
        	break;
